...
2. 目标是让每个冻结任务至少在两个条件下各出现一次。
3. 若样本数正好为 `6`：
   - `T1_A1_width` 各跑 `1` 次 baseline / process-guided
   - `T2_A1_bug_01` 各跑 `1` 次 baseline / process-guided
   - `T3_A2_bug_01` 各跑 `1` 次 baseline / process-guided
4. 若样本数不足 `6`，优先保留两个 bug-repair 任务的对照。
...
## 11. 下一步最小动作
1. 中期后先用 `T2_A1_bug_01` 和 `T3_A2_bug_01` 跑首轮 `4` 人试用。
2. 试用结束后先回填 `metrics/pilot_session_log_template_v0.csv` 的正式版本。
3. 若首轮可行，再补 `T1_A1_width` 并完善对照说明。
