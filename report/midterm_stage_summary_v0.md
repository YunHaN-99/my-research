...
## 1. 统一口径摘要
截至 `2026-04-12`，当前阶段应统一写成：“A1 / A2 首轮主案例闭环已完成；当前中期重点是收束模板库、检查清单、错误分类、题库页和原型入口。” 这不是“项目全部完成”，也不是“A2 原计划未做”，而是“首轮主案例已经闭环，当前优先任务是把中期材料写成一套不冲突的口径”。
...
- 试用准备：baseline vs 过程化流程的小样本试用 `v0` 方案、参与者表单、评分表、记录模板和 CSV 数据结构已预埋。
...
### 还没做什么
- 题库条目数已经补到中期够用，但 A3 / A4 首发条目仍未进入固定协议 / 正式实验主线。
- “输入题目 -> 结构化表示 -> 代码骨架 -> 自检 -> 结果记录”的最小 CLI 原型已建立，但更完整的统一演示版本尚未形成。
- 小范围试用、学生反馈和后续量化对照的实际数据采集尚未启动，但试用方案与数据结构已经准备好。
...
### A2
- 当前主案例：低秩图像任务族下、已完成范围收束并完成主案例闭环的 `hw2-op2/src/chapter5_rslt.py::rslt_inpainting(...)` 灰度图像修复主案例
- 研究用途：验证低秩图像任务中的 fixed-protocol 评测、bug-repair、expanded-scope 和 fresh-generation 复现

### A2 范围收束说明
- `Timeline.docx` 早期把 A2 宽表述为 “SVD 图像压缩”。
- 进入仓库实施阶段后，为提高可复现性和可评分性，A2 被收束为低秩图像任务族下、最终聚焦 `hw2-op2/src/chapter5_rslt.py::rslt_inpainting(...)` 的灰度图像修复主案例。
...
- `report/a2_expanded_scope_selected_entries_v1.md`：A2 expanded-scope 代表案例的正式入库说明
...
- fixed-protocol 的 4 个 case 在 15 次运行中保持完全一致的 `(PSNR, SSIM, RSE)`，说明当前主差异仍主要体现在过程质量、过程化自检与可复查性。
...
### 尚未满足的部分
- 题库已经具备中期够用的条目规模，但 A3 / A4 首发条目还未接入固定协议 / 实验链路。
- “输入题目 -> 结构化表示 -> 代码骨架 -> 自检 -> 结果记录”的最小 CLI 原型已完成，但更完整的统一演示版本还未形成。
- 小范围试用和学生反馈的实际招募、执行与数据回填尚未开始，但表单、评分表、记录模板与 CSV 数据结构已预埋。

## 8. 中期前最该补的材料
当前最优先的不是继续加同类型 run，而是把已有资产变成中期可提交材料：

1. Timeline 口径对齐说明
2. 模板库、检查清单与错误分类规则的收束版
3. 正式 task-bank 索引页、A2 expanded-scope 代表案例与 A3 / A4 首发条目
4. CLI 原型入口与原型说明
5. 小样本试用方案、表单、评分表和记录模板
6. 阶段小结与附件清单
...
5. 中期之后先做 `1` 次内部 dry run，再用 `T2_A1_bug_01` / `T3_A2_bug_01` 跑首轮 `4` 人小样本。
6. 首轮试用稳定后，再把 A3 / A4 首发条目推进到固定协议与正式实验，并继续补原型。
